api = 2.0
core = 8.x
projects[drupal][version] = 8.5.3

projects[address][version] = 1.3
projects[admin_toolbar][version] = 1.22
projects[adminimal_admin_toolbar][version] = 1.5
projects[better_exposed_filters][version] = 3.0-alpha3
projects[config_update][version] = 1.5
projects[csv_serialization][version] = 1.2
projects[devel][version] = 1.2
projects[entity_print][version] = 2.0
projects[features][version] = 3.5
projects[field_group][version] = 3.0-beta1
projects[oauth2_client][version] = 1.0-alpha1
projects[views_autocomplete_filters][version] = 1.2
projects[adminimal_theme][version] = 1.3
projects[bootstrap][type] = module
projects[ew_quickbooks][version] = 8.5.3
projects[ew_quickbooks][download][type] = git 
projects[qb_theme][type] = theme
projects[qb_theme][download][type] = git 

